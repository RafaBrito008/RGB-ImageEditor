...
class ImageEditorApp:
    ...
    def crear_canal(self, frame, texto, color):
        ...
        scale = tk.Scale(frame_canal, from_=0, to=2, resolution=0.01, orient=tk.HORIZONTAL, command=self.modificar_imagen, length=300)
        scale.set(1)
        scale.pack(pady=5)
    ...
    def modificar_imagen(self, *args):
        if self.imagen_original is not None:
            factor_r = self.scale_r.get()
            factor_g = self.scale_g.get()
            factor_b = self.scale_b.get()

            imagen_modificada = self.imagen_original.copy()
            r, g, b = imagen_modificada.split()

            r = r.point(lambda x: min(255, x * factor_r))
            g = g.point(lambda x: min(255, x * factor_g))
            b = b.point(lambda x: min(255, x * factor_b))
